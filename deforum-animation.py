...
api_url = f'{server}/api/public/v1/animation'
...
def main():
    ...
    response = requests.post(api_url, headers=headers, json={
        ...
        # --- optional args: ---
        # promptStyle: 'Lowpoly',
        # motionPreset: 'X-Classic-Stop-Motion',
        # aspectRatio: '16:9',
    ...
    tracking_url = f"{server}{data['links']['animation']}"
    ...
    while True:
        ...
        preview_url = tracking_data['links']['preview'] if ('preview' in tracking_data['links']) else None

        print(f"Job status: {tracking_data['status']}.{f" Latest preview: {preview_url}" if preview_url else ""} Waiting for completion...")
    ...
    if tracking_data['status'] != 'succeeded':
        ...
        print(f"Job succeeded. Access the result here: {tracking_data['links']['final']}")
